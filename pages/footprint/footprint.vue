<template>
	<view>
		<top :isNavbar="true" :isBack="true" title="我的足迹" class="z" bgc="#fff"></top>
		<view class="m-t20">
			<view class="" v-if="dataList.length>0">
				<view class="flex-a-center f-wrap a-center">
...
<script>
	export default {
		...
		methods: {
			...
			del(item) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							that.$utils.removeHistory('browser', item, 'gid')
							that.getData()
						}
					}
				})
			...
			//获取浏览历史记录
			getData() {
				let brower = this.$utils.getHistory('browser')
